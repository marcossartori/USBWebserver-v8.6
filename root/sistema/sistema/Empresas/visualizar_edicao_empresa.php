...
if ($objetoConexao->estaConectado() == false){
	...
	if ($objetoConexao->estaComBancoSelecionado()==false) {
		...
		$idGet = $_GET['id'];
		$sql = "SELECT * FROM contratos, tab_empresa WHERE tab_empresa.empresa = contratos.empresa AND contratos.id = $idGet";
...
<tr>
 ...
 if ($row ["cod_estados"] == $dado_adm['estado']) $_SESSION['estado']  =  $row ["sigla"]; 
...
<tr>
 ...
 if ($row ["cod_cidades"] == $dado_adm['Cidade'])  $_SESSION['cidade']  =  $row ["nome"]; 
    ...
    <tr>
	 ...
	 if($_SESSION["Acesso"] == 'Terceirizados' and  $dado_adm['vinculo'] == $_SESSION["Acesso"]){
		...
		echo "<a href='"; 
		echo "http://localhost/sistema/Empresas/edit_empresa.php?id=";
		echo $dado_adm['id'];
		echo "'>";
		echo " Editar  ";  echo "</a>";
	}
