...
<option value="<?php echo $reg["id_unidade"];?>" <?php if ($reg["id_unidade"] == $dado_adm['unidade']) { echo "SELECTED"; } else { echo ""; }; ?>><?php echo $reg["unidade"];?></option>>
                   ...
                   <tr>
      <th width="226" align="right" scope="row" >Cargo Chefia</th>

				<td><select id="cargo_chefia" name="cargo_chefia">
					 <option value="">Selecione
					
					<?php
						$sql = "SELECT cargo_chefia,id_chefia FROM tab_cargo_chefia WHERE cargo_chefia <> 'null' ORDER BY cargo_chefia ASC";
						$executar = mysql_query($sql) or die(mysql_error());
						while($reg = mysql_fetch_array($executar))
						{
							$id = $reg['cargo_chefia'];
					?>
<option value="<?php echo $reg["id_chefia"];?>" <?php if ($reg["id_chefia"] == $dado_adm['cargo_chefia']) { echo "SELECTED"; } else { echo ""; }; ?>><?php echo $reg["cargo_chefia"];?></option>>
					<?php
					}
					?>
				</select> </td>
