...
<option value="<?php echo $reg["unidade"];?>" <?php if ($reg["unidade"] == $dado_adm['unidade']) { echo "SELECTED"; } else { echo ""; }; ?>><?php echo $reg["unidade"];?></option>>
      ...
      <th width="226" align="right" scope="row" >Cargo Chefia</th>
				...
				<td><select id="cargo_chefia" name="cargo_chefia">
					...
					<?php
						$sql = "SELECT cargo_chefia FROM tab_cargo_chefia WHERE cargo_chefia <> 'null' ORDER BY cargo_chefia ASC";
						$executar = mysql_query($sql) or die(mysql_error());
						while($reg = mysql_fetch_array($executar))
						{
							$id = $reg["cargo_chefia"];
					?>
<option value="<?php echo $reg["cargo_chefia"];?>" <?php if ($reg["cargo_chefia"] == $dado_adm['cargo_chefia']) { echo "SELECTED"; } else { echo ""; }; ?>><?php echo $reg["cargo_chefia"];?></option>>
					<?php
					}
					?>
				</select> </td>
